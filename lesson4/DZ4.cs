...
namespace lesson4
{
    class DZ4
    {
        ...
        static string GetFullName(string firstName, string lastName, string patronymic) // метод к заданию 1
        {
            return $"{lastName} {firstName} {patronymic}";
        }

        static int Sum(string str) //метод к заданию 2
        {
        ...
        static Season? SetSeason(int monthNumber) //метод к заданию 3
        {   // Вопрос по выполнению: возврат nullable норм или лучше так не делать?
            //                       в данном случае использовать лучше if или switch?
            switch (monthNumber)
            {
                case 1:
                case 2:
        ...
        static string ShowSeason(Season? season) //еще метод к заданию 3
        {
            switch (season)
            {
                case Season.winter:
                ...
                switch (userInput)
                {
                    ...
                    case "4":
                        {// TODO (*) Написать программу, вычисляющую число Фибоначчи для заданного значения рекурсивным способом
                            ...
                            do
                            {
                                Console.WriteLine("Введите положительное число для вычисления ряда Фибоначчи: ");
        ...
        static uint FibonacciSeq(uint n) //к заданию 4, рекурсивный метод
        {
            if (n == 0)
                return n;

            return n + FibonacciSeq(n - 1);
        }
    }
}
